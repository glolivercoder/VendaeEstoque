...
(function() {
  console.log('Iniciando injeção direta do carrinho...');
  ...
  function createCart() {
    ...
    if (document.getElementById('direct-cart-button')) {
      return;
    }
    
    console.log('Criando carrinho direto...');
    ...
    itemCounter.id = 'direct-cart-counter';
    ...
    console.log('Carrinho direto criado com sucesso!');
  }
  ...
  function openCart() {
    // Obter os itens do contexto
    const context = window.__APP_CONTEXT__;
    if (context && context.items && context.selectedItems) {
      // Sincronizar os itens
      syncCartItems(context);
    }
    ...
    cartPopup.id = 'direct-cart-popup';
  ...
  function closeCart() {
    const cartPopup = document.getElementById('direct-cart-popup');
    if (cartPopup) {
      cartPopup.remove();
    }
    
    const paymentPopup = document.getElementById('direct-payment-popup');
    if (paymentPopup) {
      paymentPopup.remove();
    }
  ...
  function syncCartItems(reactContext) {
    try {
      ...
      updateItemCounter(cartItems.length);
  ...
  function updateItemCounter(count) {
    const counter = document.getElementById('direct-cart-counter');
    if (!counter) return;
  ...
  function showPaymentOptions() {
    ...
    paymentPopup.id = 'direct-payment-popup';
  ...
  // Função para monitorar o contexto React
  function monitorContext() {
    try {
      ...
      console.error('Erro ao monitorar contexto:', error);
    }
  }
  
  // Criar o carrinho quando o DOM estiver pronto
  ...
  // Monitorar o contexto periodicamente
  setInterval(monitorContext, 500);
})();
