...
export async function checkWooCommerceConfig() {
  ...
  try {
    ...
    try {
      // Obter credenciais do WordPress do localStorage
      const wpUsername = localStorage.getItem('wp_username');
      const wpPassword = localStorage.getItem('wp_password');

      // Verificar se temos credenciais de WordPress
      if (!wpUsername || !wpPassword) {
        console.log('Credenciais de WordPress não encontradas no localStorage.');
        throw new Error('Credenciais de WordPress não encontradas');
      }
