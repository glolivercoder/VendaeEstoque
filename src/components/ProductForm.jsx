...
const ProductForm = ({
  ...
  // Inicializar o array de imagens quando o componente é montado ou quando newItem muda
  useEffect(() => {
    const initialImages = [];
    if (newItem.image) {
      initialImages.push(newItem.image);
    }
    if (newItem.additionalImages && Array.isArray(newItem.additionalImages)) {
      initialImages.push(...newItem.additionalImages);
    }
    setImages(initialImages);
    setPreviewImage(newItem.image || null);
    setCurrentImageIndex(0);
  }, [newItem.id]); // Só executa quando o ID do item muda (novo item)
