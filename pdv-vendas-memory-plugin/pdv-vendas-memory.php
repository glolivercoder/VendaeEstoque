...
class PDV_Vendas_Increase_Memory {
    ...
    public function __construct() {
        ...
        // Otimizar configurações do WooCommerce se estiver ativo
        add_action('plugins_loaded', array($this, 'optimize_woocommerce'));
        ...
        // Registrar ativação e desativação do plugin
    ...
    public function plugin_activation() {
        ...
        if (class_exists('WooCommerce')) {
            // Limpar caches do WooCommerce
            if (function_exists('wc_cache_helper')) {
                wc_cache_helper()->invalidate_cache_group('products');
            }
        }

        // Adicionar opção para indicar que o plugin está ativo
        add_option('pdv_vendas_memory_active', true);
    }
    ...
    public function plugin_deactivation() {
        // Remover opção que indica que o plugin está ativo
        delete_option('pdv_vendas_memory_active');
    }
    ...
    public function optimize_woocommerce() {
        ...
        // Aumentar limite de produtos por página na API
        ...
        add_filter('woocommerce_product_data_store_cpt_get_products_query', function($query, $query_vars) {
            if (isset($query_vars['limit'])) {
                $query['posts_per_page'] = $query_vars['limit'];
            }
            return $query;
        }, 10, 2);

        // Desativar transientes para produtos (pode melhorar o desempenho em sites com muitos produtos)
        add_filter('woocommerce_product_get_price', function($price, $product) {
            $product->update_meta_data('_transient_timeout_wc_product_children_' . $product->get_id(), time() + DAY_IN_SECONDS);
            return $price;
        }, 10, 2);
    }
    ...
    public function increase_php_memory() {
        ...
        // Aumentar limites de tempo de execução para operações WooCommerce
        @set_time_limit(300); // 5 minutos
        @ini_set('max_execution_time', '300');

        // Aumentar limites de upload para imagens de produtos
        @ini_set('upload_max_filesize', '64M');
        @ini_set('post_max_size', '64M');

        // Otimizar configurações de cache do WordPress
        if (!defined('WP_CACHE')) {
...
define('WP_MAX_MEMORY_LIMIT', '1024M');</pre>
            ...
            <div class="card">
                <h2>Otimizações do WooCommerce</h2>

                <p>Este plugin também otimiza as seguintes configurações do WooCommerce:</p>

                <ul>
                    <li>Aumenta o limite de produtos por página na API REST para 100</li>
                    <li>Otimiza consultas de produtos para melhor desempenho</li>
                    <li>Gerencia caches de produtos para evitar sobrecarga de memória</li>
                    <li>Aumenta limites de upload para imagens de produtos (64MB)</li>
                    <li>Aumenta tempo máximo de execução para 5 minutos</li>
                </ul>

                <p>Status do WooCommerce: <strong><?php echo class_exists('WooCommerce') ? 'Ativo' : 'Inativo'; ?></strong></p>
            </div>
...
define('WP_MAX_MEMORY_LIMIT', '1024M');</pre>
                ...
                <p>Você também pode adicionar as seguintes linhas ao arquivo <code>.htaccess</code> para aumentar os limites do PHP:</p>

                <pre>php_value memory_limit 1024M
